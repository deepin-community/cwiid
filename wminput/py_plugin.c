...
static struct PyModuleDef moduledef = {
  ...
  "wmplugin",     /* m_name */
  "wminput plugin interface",  /* m_doc */
  -1,                  /* m_size */
...
int py_init(void)
{
	...
	if (!(PyWmPluginModule = PyModule_Create(&moduledef))) {
		PyErr_Print();
		goto ERR_HND;
	}
...
static int py_plugin_info(struct plugin *plugin, PyVarObject *info)
{
	...
	for (i=0; i < plugin->info->button_count; i++) {
		...
		if (!(plugin->info->button_info[i].name = PyBytes_AsString(PyObj))) {
			PyErr_Print();
			Py_DECREF(PyObj);
			goto ERR_HND;
		}
